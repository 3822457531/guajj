...
const routes: { test: (p: string) => boolean; title: string; subtitle: string }[] = [
  ...
  { test: (p) => matchesAdminRoute(p, "/search-analytics"), title: "搜索统计", subtitle: "全网搜索明细、每日/每周热搜" },
  ...
  { test: (p) => matchesAdminRoute(p, "/guapi-shop"), title: "瓜皮商城", subtitle: "套餐、微信/支付宝订单与查单履约" },
  { test: (p) => matchesAdminRoute(p, "/agent"), title: "代理分销", subtitle: "代理套餐、提成比例、提现审核" },
  { test: (p) => matchesAdminRoute(p, "/sms"), title: "接码管理", subtitle: "LubanSMS 配置、瓜皮定价与用户充值" },
  { test: (p) => matchesAdminRoute(p, "/media"), title: "媒体库", subtitle: "上传与管理图片、视频资源" },
  { test: (p) => matchesAdminRoute(p, "/storage"), title: "存储监控", subtitle: "OSS 全量浏览、排序与批量删除；首页资源同步硬删 index" },
